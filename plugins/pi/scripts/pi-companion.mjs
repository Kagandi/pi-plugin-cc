...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/pi-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!piStatus.available) {
    nextSteps.push("Install Pi. Ensure the `pi` CLI is available on your PATH.");
  }

  return {
    ready: nodeStatus.available && piStatus.available,
    node: nodeStatus,
    pi: piStatus,
...
function ensurePiAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Pi CLI is not installed or is missing required runtime support. Ensure `pi` is available on your PATH, then rerun `/pi:setup`.");
  }
}
...
function validateNativeReviewRequest(target, focusText) {
  if (focusText.trim()) {
    throw new Error(
      `\`/pi:review\` now maps directly to the built-in reviewer and does not support custom focus text. Retry with \`/pi:adversarial-review ${focusText.trim()}\` for focused review instructions.`
    );
  }

  const nativeTarget = buildNativeReviewTarget(target);
  if (!nativeTarget) {
    throw new Error("This `/pi:review` target is not supported by the built-in reviewer. Retry with `/pi:adversarial-review` for custom targeting.");
  }
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review") {
    ...
    // Native review: spawn pi in read-only mode with the review context
    const reviewContext = collectReviewContext(request.cwd, target);
    const prompt = `Perform a code review of the following changes.
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestThread) {
      throw new Error("No previous Pi task thread was found for this repository.");
    }
...
async function handleCancel(argv) {
  ...
  const interrupt = interruptPiProcess(job.pid);
  if (interrupt.attempted) {
    appendLogLine(
      job.logFile,
      interrupt.interrupted
        ? `Requested Pi process interrupt for pid ${job.pid}.`
        : `Pi process interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
